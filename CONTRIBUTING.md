...
- Is there a Github issue for the work you’re doing, going to do, or want someone else to do? Create one if not. [Learn how](https://help.github.com/articles/creating-an-issue/).
  ...
  - Collaborators should add appropriate labels to issues [Learn how](https://help.github.com/articles/creating-and-editing-labels-for-issues-and-pull-requests/).
    ...
    - Add ‘state’ labels to indicate whether it is ready to be worked on or in progress. State labels are synced with [waffle.io](https://waffle.io/nicholas-b-carter/usptostrap-sass-gulp), which can _optionally_ be used to manage and view issues. 
...
- Update the `baseurl` property in `/_config.yml` to `/usptostrap-sass-gulp/1.x` (for a major bump, it would be `/usptostrap-sass-gulp/2.x`)
...
- Revert the `baseurl` property in `/_config.yml` back to `/usptostrap-sass-gulp`
- Revert the `includePreviews` property in `/_config.yml` back to `true`
- Commit your changes.
- Add the release to https://github.com/nicholas-b-carter/usptostrap-sass-gulp/releases
